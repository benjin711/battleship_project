...
void Spielfeld::endposition(std::vector<std::pair<int, int>>tmpVector, std::pair<int, int>pos, int numbship)
{
...
void Spielfeld::setShip(std::vector<std::pair<int, int>>ship)

{
    ...
    for(quint8 i=0; i<ship.size(); i++)
    {
        ...
        quint8 x2 = ship[i].first;
        quint8 y2 = ship[i].second;
...
void Spielfeld::randomPositions(std::vector<std::pair<int, int> >vector)
{

    for(uint j=0; j<(vector.size()-25); j++)
    {
        int x = vector[j].first;
        int y = vector[j].second;
        auto btn = ui->gridLayout_1->itemAtPosition(x, y)->widget();
        btn->setStyleSheet("background-color: rgb(179, 255, 230)");
    }

    for(uint j=(vector.size()-25); j<(vector.size()-21); j++)
    {
        int x = vector[j].first;
        int y = vector[j].second;
        auto btn = ui->gridLayout_1->itemAtPosition(x, y)->widget();
